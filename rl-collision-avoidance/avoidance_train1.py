...
from avoidance_world1 import GazeboWorld
...
def run(comm, env, policy, policy_path, action_bound, optimizer):
    ...
    global_update = 0
    global_step = 0
    env_index=env.get_env_index()


    for id in range(MAX_EPISODES):
        env.reset_pose()#新回合开始，随机重置当前robot的位姿

        env.generate_goal_point()#获取目标位置
        terminal = False
        ep_reward = 0
        step = 1

        obs = env.get_laser_observation()
        obs_stack = deque([obs, obs, obs])#前三帧的雷达观测信息
        ...
        if env.index == 0:
            if global_update != 0 and global_update % 20 == 0:
                torch.save(policy.state_dict(), policy_path + '/flGazebo1_{}'.format(global_update))
...
if __name__ == '__main__':
    ...
    env = GazeboWorld(512, index=rank, num_env=NUM_ENV)
    reward = None
